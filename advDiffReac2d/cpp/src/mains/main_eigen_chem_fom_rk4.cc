...
int main(int argc, char *argv[]){
  ...
  scalar_t diffusion = 0.001;
  scalar_t chemReac  = 10.;

  // store cmd line args
  const std::string meshFile = "mesh50.dat";
  const auto dt = 0.005;
  ...
  src_fnct_t srcFunctor(chemReac);
  ...
  if (do_print){
    ...
    auto vx = appObj.getVx();
    auto vy = appObj.getVy();
    file.open( "uv.txt" );
    for(auto i=0; i < vx.size(); i++){
      file << std::setprecision(14) << vx[i] << " " << vy[i];
      file << std::endl;
    }
    file.close();
  }
